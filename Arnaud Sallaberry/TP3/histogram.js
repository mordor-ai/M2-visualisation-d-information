...
function updateHistX(att){
	//alert(att);
	attHist= att;
	scaleXHist.domain([d3.min(dataHist, function (d) {
		return	d[attHist];
	...
	for(i=0 ; i<nbClasses ; i++){
		...
		for(j=0 ; j<dataHist.length ; j++){
			if(parseFloat(dataHist[j][attHist])>=minV && 
			parseFloat(dataHist[j][attHist])<=maxV &&
			( dataHist[j].selected  || nothingSelected==0)){
